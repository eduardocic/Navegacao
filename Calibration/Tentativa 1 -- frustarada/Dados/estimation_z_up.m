...
for k = 2:size(index, 2)
    ...
    % Atualização dos estados.
    fy_hat{k} = phi_k_2d * fy_hat{k-1} + K{k} * (ym(k) - C_2d * phi_k_2d * fy_hat{k-1});
end
...
% for k = 2:size(index, 2)
%     % Equação de Riccati.
%     [P_3d{k}, K{k}] = kalman_calib(phi_k_3d, P_3d{k-1}, Q_k_3d, R_k_z, C_3d, I_3d);   
%     
%     % Atualização dos estados.
%     fz_hat{k} = phi_k_3d * fz_hat{k-1} + K{k} * ((zm(k)+2*g) - C_3d * phi_k_3d * fz_hat{k-1});
% end
...
for k = 2:size(index, 2)
    ...
    % Atualização dos estados.
    fz_hat{k} = phi_k_2d * fz_hat{k-1} + K{k} * ((zm(k)+2*g) - C_2d * phi_k_2d * fz_hat{k-1});
end
...
plot(index, zm);
hold on;
plot(index, (-2*g*ones(1,1000) + bz -g*S_1_z), 'r');


save('z_up_save.mat', 'fx_hat', 'fy_hat', 'fz_hat');